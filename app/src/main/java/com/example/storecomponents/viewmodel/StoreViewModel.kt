...
data class StoreUser(
    ...
    companion object {
        fun fromJson(obj: JSONObject): StoreUser = StoreUser(
            ...
            // normalizar email al cargar (trim + lowercase)
            email = obj.optString("email", null)?.trim()?.lowercase(),
...
open class StoreViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun addUser(name: String, role: String, email: String? = null, password: String? = null, direccion: String? = null) {
        ...
        val u = StoreUser(newId, name, role, false, normalizedEmail, password, direccion)
        _users.add(u)
        saveUsersToPrefs()
    }
    ...
    fun updateUser(id: Long, name: String, role: String, email: String? = null, password: String? = null, direccion: String? = null) {
        ...
        if (idx >= 0) {
            val u = _users[idx]
            // Sólo sobrescribir campos que no son nulos (preservar si se pasa null)
            val newEmail = email?.trim()?.lowercase() ?: u.email
            val newPassword = password ?: u.password
            val newDireccion = direccion ?: u.direccion
            _users[idx] = u.copy(name = name, role = role, email = newEmail, password = newPassword, direccion = newDireccion)
            saveUsersToPrefs()
        }
    }
    ...
    fun login(email: String, password: String): StoreUser? {
        ...
        return _users.firstOrNull { it.email != null && it.email == emailNorm && it.password == password }
    }

    fun getUserByEmail(email: String): StoreUser? {
        val emailNorm = email.trim().lowercase()
        return _users.firstOrNull { it.email != null && it.email == emailNorm }
    }

    // Registrar y logear en un único paso. Devuelve Pair(success, mensaje opcional).
    fun registerAndLogin(name: String, role: String, email: String?, password: String?, direccion: String? = null): Pair<Boolean, String?> {
        val emailNorm = email?.trim()?.lowercase()
        if (emailNorm == null || emailNorm.isBlank()) return Pair(false, "Email inválido")
        if (_users.any { it.email != null && it.email == emailNorm }) return Pair(false, "Email ya existe")
        val newId = if (_users.isEmpty()) 1L else (_users.maxOf { it.id } + 1)
        val u = StoreUser(newId, name, role, false, emailNorm, password, direccion)
        _users.add(u)
        saveUsersToPrefs()
        // intentar login inmediato
        return if (login(emailNorm, password ?: "") != null) Pair(true, null) else Pair(false, "Registro OK pero login falló")
    }
